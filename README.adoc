...
| *Idris 2*
| Proofs-as-types with Quantitative Type Theory (QTT). Dependent types + quantities (0, 1, ω) = memory safety, resource management, and erasure control proven at compile time. If it compiles, it's correct. First-class totality checking.
|===

NOTE: Idris 2 has a steep learning curve but excellent resources (Edwin Brady's "Type-Driven Development"). Use when correctness is non-negotiable and you want the most expressive type system available.
