...
RSR (Rhodium Standard Repository) begins as a template—a collection of best practices, CI workflows, and consent declarations. By v10.0, it becomes *infrastructure*: a self-enforcing trust network where security, consent, and provenance are not optional policies but cryptographically verifiable properties of the software itself.
...
Transform RSR from passive policies to active enforcement. Projects don't just *declare* compliance—they *prove* it.
...
== Phase 2: Consent Protocol (v3.0 → v5.0)
...
├── issue      # Request a consent token
├── verify     # Verify a token
├── revoke     # Revoke granted consent
└── audit      # Audit trail of consent usage
----
...
== Phase 3: Provenance Chain (v6.0 → v8.0)
...
[source]
----
Source → Build → Attest → Publish → Verify
   ↓        ↓        ↓         ↓        ↓
...
For critical code paths, move beyond testing to mathematical proof. The gold standard is *Idris 2*—where proofs ARE types and Quantitative Type Theory gives you fine-grained control over resource usage—with SPARK and proof assistants as pragmatic alternatives.

=== The Idris 2 Philosophy

[quote, Core Insight]
____
In Idris 2, if your code compiles, your proofs are valid. The type system IS the theorem prover. Quantitative Type Theory means you control exactly how many times each value is used.
____

Idris 2 with Quantitative Type Theory (QTT) combines:

* *Dependent types*: Types that depend on values (e.g., `Vect n a` — a vector of exactly `n` elements)
* *Quantitative types*: Resources annotated with usage (0 = erased, 1 = linear, ω = unrestricted)
* *Totality checking*: First-class guarantee that functions terminate and cover all cases
* *Theorem proving*: Propositions encoded as types, proofs as programs

This means:

* Memory safety with fine-grained control—*proven*
* Compile-time proofs that vanish at runtime (0 quantity)—*zero overhead*
* Resource cleanup guaranteed (1 quantity)—*proven*
* If it compiles, the invariants hold
* If it's total, it terminates
...
| Platinum (Idris 2)
| Proofs-as-types + QTT
| Idris 2 dependent types + quantities—proof IS compilation

| Platinum (Alt)
| Machine-checked proofs
| Coq, Lean, Isabelle (external provers)
|===

=== Why Idris 2 Over External Provers

[cols="1,2,2", options="header"]
|===
| Aspect | Idris 2 | Coq/Lean/Isabelle

| Proof location
| IN the types (inseparable from code)
| BESIDE the code (separate proof files)

| Proof maintenance
| Automatic (change code = change proof)
| Manual (proofs can drift from code)

| Resource control
| QTT quantities (0, 1, ω) built-in
| Must encode manually or use plugins

| Erasure
| 0-quantity proofs vanish at runtime
| Extraction phase can be unpredictable

| Learning curve
| Steep (but one system to learn, great book)
| Steep (two systems: code + prover)
|===

=== Integration Points

* CI compiles Idris 2 code (compilation = verification)
* Quantity checking enforces resource safety (1 = linear)
* 0-quantity proofs exist at compile time, erased at runtime
* Totality checker ensures termination
* Falls back to SPARK/Coq for legacy or team-pragmatic code
...
| Idris 2 CI Integration
| Compile Idris 2 code, verify proofs via compilation, check totality
| P0

| Idris 2 Templates
| RSR templates for common patterns (safe arrays, protocols, state machines)
| P1

| SPARK CI Integration
| Validate Ada/SPARK contracts in CI (pragmatic fallback)
| P1

| TLA+ Toolbox
| CI for TLA+ model checking (specification layer)
| P2

| Coq/Lean Integration
| Proof verification in CI (when external provers needed)
| P2

| Idris 2-to-Rust FFI
| Safe interop between Idris 2 (via RefC) and Rust codebases
| P2
...
RSR becomes invisible infrastructure—the default way software is built.
...
* Idris 2 dependent types ↔ Rust invariants ↔ SPARK contracts ↔ Haskell types
* Automatic property translation
* Cross-language proof portability
* Idris 2 as the "proof source" with multiple backends (Scheme, RefC, JS)
...
| v9.0 (Q1 2030)
| Formal Verification
| Idris 2 proofs-as-types + QTT, SPARK/TLA+ fallbacks
...
* https://www.idris-lang.org[Idris 2] — Proofs-as-types with Quantitative Type Theory
* https://github.com/idris-lang/Idris2[Idris 2 GitHub] — Active development
...
[.text-center]
Version History: 2025-12-24 (v0.4.0) • Switched to Idris 2 (QTT) as Tier 0 verification language
