...
For critical code paths, move beyond testing to mathematical proof. The gold standard is *ATS*—where proofs ARE types—with SPARK and proof assistants as pragmatic alternatives.

=== The ATS Philosophy

[quote, Core Insight]
____
In ATS, if your code compiles, your proofs are valid. The type system IS the theorem prover.
____

ATS (Applied Type System) combines:

* *Dependent types*: Types that depend on values (e.g., `array(n)` where `n` is a compile-time value)
* *Linear types*: Resources that must be used exactly once (no leaks, no double-free, proven at compile time)
* *Theorem proving*: Propositions encoded as types, proofs as programs

This means:

* Memory safety without garbage collection—*proven*
* Array bounds checked at compile time—*proven*
* Resource cleanup guaranteed—*proven*
...
| Platinum (ATS)
| Proofs-as-types
| ATS dependent/linear types—proof IS compilation
...
=== Why ATS Over External Provers

[cols="1,2,2", options="header"]
|===
| Aspect | ATS | Coq/Lean/Isabelle
...
| Runtime overhead
| Zero (compiles to C)
| Extraction can introduce overhead

| Learning curve
| Steep (but one system to learn)
| Steep (two systems: code + prover)
|===

=== Integration Points

* CI compiles ATS code (compilation = verification)
* Linear type checking enforces resource safety
* Dependent types encode invariants in function signatures
...
| ATS CI Integration
| Compile ATS code with `patscc`, verify proofs via compilation
| P0

| ATS Templates
| RSR templates for common patterns (safe arrays, resources, protocols)
| P1
...
| ATS-to-Rust FFI
| Safe interop between ATS and Rust codebases
| P2
...
* ATS dependent types ↔ Rust invariants ↔ SPARK contracts ↔ Haskell types
* Automatic property translation
* Cross-language proof portability
* ATS as the "proof source" with extraction to other languages
...
| v9.0 (Q1 2030)
| Formal Verification
| ATS proofs-as-types, SPARK/TLA+ fallbacks
...
* https://www.ats-lang.org[ATS (Applied Type System)] — Proofs-as-types, dependent + linear types
...
[.text-center]
Version History: 2025-12-24 (v0.3.0) • Added ATS as Tier 0 verification language
